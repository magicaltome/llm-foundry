...
class OpenAITokenizerWrapper:
    ...
    def encode(self, x, add_special_tokens=False):
        if isinstance(x, str):
            ...
            return {
                "input_ids": self.tokenizer.encode_batch(x, allowed_special={'<|endoftext|>'})
            }
    ...
    def construct_logit_tensor(self, logprobs):
        tensor = torch.tensor([min(logprobs.values()) - 1]*(self.pad_token_id+1))
        for k in logprobs:
...
class OpenAICausalLMEvalWrapper(ComposerModel):
    ...
    def __init__(self, model_name, tokenizer, chat_model=False):
    ...
    def eval_forward(self, batch, outputs: Optional[Any] = None):
        ...
        for tokens, cont_idxs in zip(batch['input_ids'], batch['continuation_indices']):
            ...
            output_logits = torch.zeros( cont_idxs[0] - 1, self.tokenizer.pad_token_id + 1)
            for i in range(len(expected_cont_tokens)):
                ...
                if not self.chat_model:
                    ...
                    output_logits = torch.cat([output_logits, tensor.reshape(1,-1)])

            output_logits = torch.cat([output_logits, torch.zeros(seqlen - output_logits.shape[0], self.tokenizer.pad_token_id + 1)])
